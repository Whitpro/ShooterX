...
class WebGPUTextureUtils {
	...
	createTexture( texture, options = {} ) {
		...
		// Check if texture is already initialized and handle gracefully
		if ( textureData.initialized ) {

			// If the texture is already initialized, just return without error
			// This can happen during device recovery or texture reuse scenarios
			console.warn( 'WebGPUTextureUtils: Texture already initialized, skipping creation:', texture.name || texture.id );
			return;
		}

		// Check if device is available
		if ( !backend.device ) {
			console.warn( 'WebGPUTextureUtils: Device not available, skipping texture creation:', texture.name || texture.id );
			return;
		}
	...
	/**
	 * Resets the initialization state of a texture.
	 * Useful during device recovery when textures need to be recreated.
	 *
	...
	resetTextureState( texture ) {
		...
		delete textureData.msaaTexture;
		delete textureData.externalTexture;
		delete textureData.textureDescriptorGPU;
		delete textureData.sampler;

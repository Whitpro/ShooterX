...
class Renderer {
	...
	constructor( backend, parameters = {} ) {
		...
		/**
		 * A callback function that defines what should happen when a device/context is recovered.
		 *
		 * @type {Function}
		 */
		this.onDeviceRecovered = this._onDeviceRecovered;
	...
	/**
	 * Default implementation of the device recovered callback.
	 *
	 * @private
	 */
	_onDeviceRecovered() {

		console.log( 'THREE.WebGPURenderer: Device recovered successfully!' );

		this._isDeviceLost = false;

		// Force recreation of all cached resources
		this._clearCachedResources();

		// Dispatch recovery event if dispatchEvent is available
		if ( typeof this.dispatchEvent === 'function' ) {
			this.dispatchEvent( { type: 'devicerecovered' } );
		}

	}

	/**
	 * Clears all cached resources when device is recovered.
	 *
	 * @private
	 */
	_clearCachedResources() {

		// Clear all cached data (with safety checks)
		if ( this._objects ) this._objects.dispose();
		if ( this._pipelines ) this._pipelines.dispose();
		if ( this._bindings ) this._bindings.dispose();
		if ( this._textures ) this._textures.dispose();
		if ( this._geometries ) this._geometries.dispose();
		if ( this._attributes ) this._attributes.dispose();

		// Reinitialize components only if backend is available
		if ( this.backend ) {
			this._objects = new RenderObjects( this, this._nodes, this._geometries, this._pipelines, this._bindings, this.info );
	...
	_renderScene( scene, camera, useFrameBufferTarget = true ) {
		...
		// Check if device recovery is in progress (WebGPU specific)
		if ( this.backend.isWebGPUBackend && this.backend.isRecovering && this.backend.isRecovering() ) {
			return;
		}

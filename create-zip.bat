@echo off
echo Creating Shooter X ZIP Package v2.0.0...
...
powershell -Command "& {Compress-Archive -Path dist\shooter-x-win32-x64\* -DestinationPath dist\ShooterX-v2.0.0.zip -Force}"
...
if errorlevel 1 (
    ...
    echo The ZIP file is located at: %CD%\dist\ShooterX-v2.0.0.zip
)

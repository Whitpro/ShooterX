...
class Wave {
    ...
    getWaveComposition(wave) {
        const totalEnemies = wave === 10 ? 20 : this.waveCounts[Math.min(wave, this.waveCounts.length - 1)];
        const enemyTypes = [];
    ...
    startWave() {
        ...
        const enemyTypes = this.getWaveComposition(this.wave);

        if (this.wave === 10) {
            if (this.engine && this.engine.bossFight) {
                this.engine.bossFight.start();
            }
        }
    ...
    update(deltaTime) {
        ...
        if (
            ...
            this.enemyManager.enemies.every(e => !e.isAlive)
        ) {
            console.log('Wave complete! Killed:', this.enemiesKilled);
            this.completeWave();
            return;
        }
    ...
    onEnemyKill(type) {
        ...
        if (this.engine && this.engine.ui) {
            this.engine.ui.updateScore(this.getCurrentState());
        }
        ...
        if (this.wave >= this.maxWave) {
            ...
            if (this.engine) {
                ...
                if (this.engine.scene) {
                    this.engine.scene.visible = false;
                }
                if (this.engine.ui) {
                    this.engine.ui.hideBossHealthBar();
                    this.engine.ui.showGameCompletionScreen();
                }
    ...
    getCurrentState() {
        const alive = this.enemyManager ? this.enemyManager.enemies.filter(e => e.isAlive).length : 0;
        return {
            wave: this.wave,
            state: this.state,

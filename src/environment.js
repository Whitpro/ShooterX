...
class Environment {
    constructor(scene) {
        ...
        console.log('Environment constructor: boundaryRadius set to', this.boundaryRadius);
        ...
        // Reusable temp objects for collision and generation math
        this._tempVertex = new THREE.Vector3();
        this._tempLocalPos = new THREE.Vector3();
        this._tempToPoint = new THREE.Vector3();
        ...
        // Create animated boundary after the world is created
        console.log('Creating animated boundary after world creation');
    ...
    reset() {
        ...
        // Clean up boundary posts
        if (this.boundaryPosts && this.boundaryPosts.length > 0) {
            this.boundaryPosts.forEach(post => {
                if (post) {
                    // Remove all children (lights, etc.)
                    while (post.children.length > 0) {
                        const child = post.children[0];
                        post.remove(child);
                    }
                    
                    this.scene.remove(post);
                    if (post.geometry) post.geometry.dispose();
                    if (post.material) post.material.dispose();
                    totalObjectsRemoved++;
                }
            });
            this.boundaryPosts = [];
        ...
        // Clean up boundary beams
        if (this.boundaryBeams && this.boundaryBeams.length > 0) {
            this.boundaryBeams.forEach(beam => {
                if (beam) {
        ...
        if (this.centerMarker) {
            ...
            if (this.centerMarker.geometry) this.centerMarker.geometry.dispose();
            if (this.centerMarker.material) this.centerMarker.material.dispose();
            this.centerMarker = null;
            totalObjectsRemoved++;
        }
        
        // Clean up old boundary wall (if it exists)
        ...
        if (this.debugArrows && this.debugArrows.length > 0) {
    ...
    createRock(x, y, z) {
        ...
        for (let i = 0; i < positions.count; i++) {
            ...
            // Apply deformation based on position
    ...
    createTree(x, z) {
        ...
        treeGroup.trunk = trunk;
        treeGroup.foliage = foliageGroup;
    ...
    addGrass() {
        ...
        while (validInstanceCount < instanceCount && attempts < maxAttempts) {
            ...
            if (!this.isOnPath(x, z)) {
                ...
                this._tempGrassQuaternion.setFromEuler(rotation);
                matrix.compose(position, this._tempGrassQuaternion, scale);
    ...
    createMapBoundaries() {
        const boundaryRadius = this.boundaryRadius + 8;
        const numBoundaryObjects = 64;

        for (let i = 0; i < numBoundaryObjects; i++) {
            const angle = (i / numBoundaryObjects) * Math.PI * 2;
            const radialJitter = (Math.random() - 0.5) * 6;
            const x = Math.cos(angle) * (boundaryRadius + radialJitter);
            const z = Math.sin(angle) * (boundaryRadius + radialJitter);

            try {
                if (i % 4 === 0) {
                    this.createFallenLog(x, 0, z);
                } else if (i % 3 === 0) {
    ...
    checkWallCollision(position) {
        ...
        if ((position.x * position.x + position.z * position.z) > boundaryRadiusSquared) {
            return true;
        }
            ...
            switch(wall.type) {
                ...
                case 'log':
                    ...
                    if (distanceToLog < wall.logRadius + playerRadius) {
                        ...
                        const localPos = this._tempLocalPos.subVectors(position, wall.collisionPoints[0]);
    ...
    pointToLineDistance(point, lineStart, direction, maxDistance) {
        const toPoint = this._tempToPoint.subVectors(point, lineStart);
        const projectionLength = toPoint.dot(direction);
        const projection = this._tempProjection.copy(direction).multiplyScalar(projectionLength);
        return toPoint.sub(projection).length();
    }

    createAnimatedBoundary() {
        debug('Creating realistic perimeter fence...');

        if (typeof this.boundaryRadius !== 'number' || Number.isNaN(this.boundaryRadius)) {
            console.error('boundaryRadius is not properly defined:', this.boundaryRadius);
            this.boundaryRadius = 65;
        }
        ...
        try {
            ...
            const fenceRadius = this.boundaryRadius + 1.5;
            const postCount = 44;
            const postHeightMin = 2.4;
            const postHeightMax = 3.2;
            const postWidth = 0.22;
            const railHeight = 0.16;
            const railDepth = 0.12;
            const railOverlap = 0.18;
            ...
            for (let i = 0; i < postCount; i++) {
                ...
                const radius = fenceRadius + radiusJitter;
                const x = Math.cos(angle) * radius;
                const z = Math.sin(angle) * radius;
                const postHeight = postHeightMin + Math.random() * (postHeightMax - postHeightMin);

                const postGeometry = new THREE.BoxGeometry(postWidth, postHeight, postWidth);
                const post = new THREE.Mesh(postGeometry, fenceMaterial.clone());
                post.position.set(x, postHeight / 2, z);
                post.rotation.y = angle + Math.PI / 2 + (Math.random() - 0.5) * 0.05;
                post.rotation.z = (Math.random() - 0.5) * 0.025;
                post.castShadow = true;
                post.receiveShadow = true;
                post.userData.isEnvironment = true;
                this.scene.add(post);
                this.boundaryPosts.push(post);
                postData.push({ angle, radius, height: postHeight });
            }

            for (let i = 0; i < postCount; i++) {
                const currentPost = this.boundaryPosts[i];
                const nextPost = this.boundaryPosts[(i + 1) % postCount];
                const currentData = postData[i];
                const nextData = postData[(i + 1) % postCount];
                const dx = nextPost.position.x - currentPost.position.x;
                const dz = nextPost.position.z - currentPost.position.z;
                const segmentLength = Math.sqrt(dx * dx + dz * dz) + railOverlap * 2;
                const midX = (currentPost.position.x + nextPost.position.x) / 2;
                const midZ = (currentPost.position.z + nextPost.position.z) / 2;
                const rotationY = Math.atan2(dz, dx);
                const lowerRailY = Math.min(currentData.height, nextData.height) * 0.38;
                const upperRailY = Math.min(currentData.height, nextData.height) * 0.7;

                const lowerRailGeometry = new THREE.BoxGeometry(segmentLength, railHeight, railDepth);
                const lowerRail = new THREE.Mesh(lowerRailGeometry, fenceMaterial.clone());
                lowerRail.position.set(midX, lowerRailY, midZ);
                lowerRail.rotation.y = rotationY;
                lowerRail.castShadow = true;
                lowerRail.receiveShadow = true;
                lowerRail.userData.isEnvironment = true;
                this.scene.add(lowerRail);
                this.boundaryBeams.push(lowerRail);

                if (i % 7 !== 0) {
                    const upperRailGeometry = new THREE.BoxGeometry(segmentLength, railHeight, railDepth);
                    const upperRail = new THREE.Mesh(upperRailGeometry, fenceMaterial.clone());
                    upperRail.position.set(midX, upperRailY, midZ);
                    upperRail.rotation.y = rotationY;
                    upperRail.castShadow = true;
                    upperRail.receiveShadow = true;
                    upperRail.userData.isEnvironment = true;
                    this.scene.add(upperRail);
                    this.boundaryBeams.push(upperRail);
                }
            }

            this._animationTime = 0;
        } catch (error) {
            console.error('Error creating boundary visualization:', error);
        }
    }
    ...
    updateBarrierLights(deltaTime, intensityFactor = 1.0) {
        ...
        this.barrierLights.forEach((light, index) => {
            if (light instanceof THREE.PointLight) {
                ...
                light.intensity = 4 * offsetPulse; // Adjusted intensity for better visibility
    ...
    spawnRandomPowerUp() {
        try {
            ...
            debug(`Spawning random power-up of type: ${type}`);
            ...
            debug(`Power-up created successfully. Power-ups count: ${this.powerUps.length}`);
        } catch (error) {
    ...
    spawnPowerUp(type) {
        if (['health', 'ammo', 'rapidfire'].includes(type)) {
            debug(`Manually spawning power-up of type: ${type}`);
            const powerUp = new PowerUp(type, this);
            this.powerUps.push(powerUp);
            debug(`Power-ups count: ${this.powerUps.length}`);
        } else {
    ...
    forceSpawnPowerUp(type = null) {
        ...
        debug(`Force spawning power-up of type: ${type}`);
